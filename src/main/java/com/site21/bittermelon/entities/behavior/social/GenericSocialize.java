...
public class GenericSocialize<E extends LivingEntity & Socializable> extends ExtendedBehaviour<E> {
    private static final MemoryTest MEMORY_REQUIREMENTS = MemoryTest.builder(4).hasMemory(MemoryModuleType.NEAREST_VISIBLE_LIVING_ENTITIES).noMemory(MemoryModuleTypeInit.SOCIALIZE_TARGET.get()).usesMemories(MemoryModuleType.LOOK_TARGET, MemoryModuleType.WALK_TARGET);
    ...
    public GenericSocialize<E> speedMod(final BiFunction<E, LivingEntity, Float> speedModifier) {
        this.speedMod = speedModifier;

        return this;
    }

    public GenericSocialize<E> messages(List<String> messages) {
    ...
    public GenericSocialize<E> closeEnoughDist(final ToIntBiFunction<E, LivingEntity> closeEnoughDist) {
    ...
    protected void start(E entity) {
        ...
        BrainUtils.setMemory(entity, MemoryModuleTypeInit.SOCIALIZE_TARGET.get(), this.partner);
        BrainUtils.setMemory(this.partner, MemoryModuleTypeInit.SOCIALIZE_TARGET.get(), entity);
    ...
    protected void tick(E entity) {
        ...
        if (entity.closerThan(this.partner, closeEnoughDist.applyAsInt(entity, partner)) && entity.tickCount == this.socializeTick) {
            ...
            BrainUtils.clearMemory(entity, MemoryModuleTypeInit.SOCIALIZE_TARGET.get());
            BrainUtils.clearMemory(this.partner, MemoryModuleTypeInit.SOCIALIZE_TARGET.get());
        }
    }
    ...
    protected void stop(E entity) {
        BrainUtils.clearMemories(entity, MemoryModuleTypeInit.SOCIALIZE_TARGET.get(), MemoryModuleType.LOOK_TARGET, MemoryModuleType.WALK_TARGET);

        if (this.partner != null)
            BrainUtils.clearMemories(this.partner, MemoryModuleTypeInit.SOCIALIZE_TARGET.get(), MemoryModuleType.LOOK_TARGET, MemoryModuleType.WALK_TARGET);
