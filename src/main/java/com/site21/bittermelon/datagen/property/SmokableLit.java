...
public record SmokableLit() implements ConditionalItemModelProperty {
    public static final MapCodec<SmokableLit> MAP_CODEC = MapCodec.unit(new SmokableLit());
    ...
    public boolean get(@NotNull ItemStack stack, @Nullable ClientLevel level, @Nullable LivingEntity entity, int seed, @NotNull ItemDisplayContext displayContext) {
        return stack.getOrDefault(LIT, false);
    }
}
