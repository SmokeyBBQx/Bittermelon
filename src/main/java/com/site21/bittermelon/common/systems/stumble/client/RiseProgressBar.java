...
public class RiseProgressBar {
    private static final ResourceLocation PROGRESS_BAR_BACKGROUND = ResourceLocation.fromNamespaceAndPath("bittermelon", "hud/progress_bar_background");
    private static final ResourceLocation PROGRESS_BAR_PROGRESS = ResourceLocation.fromNamespaceAndPath("bittermelon", "hud/progress_bar_progress");
    private static final ResourceLocation STUN_BAR_PROGRESS = ResourceLocation.fromNamespaceAndPath("bittermelon", "hud/stun_bar_progress");

    private static final int BAR_WIDTH = 182;
    private static final int BAR_HEIGHT = 5;
    private static final int MAX_STUN_TIME = 40;

    private static void renderProgressBar(@NotNull GuiGraphics guiGraphics, int x, int y) {
        float ticksHeld = RiseKeyHandler.getTicksHeld();

        guiGraphics.blitSprite(RenderPipelines.GUI_TEXTURED, PROGRESS_BAR_BACKGROUND, x, y, BAR_WIDTH, BAR_HEIGHT);

        if (ticksHeld > 0) {
            int progressWidth = Math.min((int) ((ticksHeld / (TICKS_REQUIRED - 2)) * BAR_WIDTH), BAR_WIDTH);
            guiGraphics.blitSprite(RenderPipelines.GUI_TEXTURED, PROGRESS_BAR_PROGRESS, BAR_WIDTH, BAR_HEIGHT, 0, 0, x, y, progressWidth, BAR_HEIGHT);
        }
    }
    ...
    private static void renderStunBar(@NotNull GuiGraphics guiGraphics, int x, int y, @NotNull Player player) {
        ...
        guiGraphics.blitSprite(RenderPipelines.GUI_TEXTURED, STUN_BAR_PROGRESS, BAR_WIDTH, BAR_HEIGHT, 0, 0, x, y, progressWidth, BAR_HEIGHT);
    }
