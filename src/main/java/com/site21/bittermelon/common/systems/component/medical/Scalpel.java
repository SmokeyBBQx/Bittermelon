...
public record Scalpel(float efficiency, Optional<Identifier> icon) implements MedicalInstrument {
    ...
    public static final Codec<Scalpel> CODEC = RecordCodecBuilder.create(instance ->
            instance.group(
                    ...
                    Identifier.CODEC.optionalFieldOf("icon").forGetter(Scalpel::icon)
            ).apply(instance, Scalpel::new));

    @Contract(pure = true)
    @Override
    public @NotNull Optional<Identifier> icon() {
        return icon;
    }
}
